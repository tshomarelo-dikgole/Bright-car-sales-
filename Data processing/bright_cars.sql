...
SELECT
  DATE_TRUNC(PARSE_TIMESTAMP('%a %b %d %Y %H:%M:%S', saledate), MONTH) AS sale_month,
  COUNT(vin) AS transaction_count
FROM
  `car_data`.`car_sales`
GROUP BY
  sale_month
...
SELECT
  DATE_TRUNC(PARSE_TIMESTAMP('%a %b %d %Y %H:%M:%S', saledate), DAY) AS sale_day,
  COUNT(DISTINCT vin) AS count_of_transactions_day
FROM`car_data`.`car_sales`
GROUP BY
  sale_day
ORDER BY
...
SELECT
  ...
  COUNT(car_sales.vin) AS count_of_transactions
FROM`car_data`.`car_sales` 
GROUP BY
  sale_year
ORDER BY
